    ...
    public class MethodNamesFromStackFramesExtractor : IMethodNamesFromStackFramesExtractor {
        ...
        private string GetMethodNameFromFrame(StackFrame stackFrame) {
            ...
            return ExtractMethodName(
                typeof(IAsyncStateMachine).GetTypeInfo().IsAssignableFrom(declaringType) ? declaringType.FullName : method.Name
            );
        }
        ...
        private string ExtractMethodName(string stringContainingMethodName) {
            ...
            stringContainingMethodName = ltPos >= 0 && gtPos > 0 ? stringContainingMethodName.Substring(ltPos + 1, gtPos - ltPos - 1) : stringContainingMethodName;
            stringContainingMethodName = UnwantedMethods.Contains(stringContainingMethodName) ? null : stringContainingMethodName;
            return stringContainingMethodName;
        }
    }
}
